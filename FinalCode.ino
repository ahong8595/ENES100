...
void loop(){
  ...
  if(Enes100.updateLocation()&&(Enes100.destination.y != 0 && Enes100.destination.x != 0)){
    ...
    forward(150);
        ...
        //turns left if both sensors measure less than 5cm
      if (readLeftDistanceSensor() <= 10.0){
        ...
        //turns left if it right sensor measures less than or equal to 5cm
        if (readLeftDistanceSensor() >= 10.0) {
            delay(200);
            if(readRightDistanceSensor() <= 10.0){
                delay(200);
        ...
        //turns right if left sensor measures less than or equal to 5cm 
            delay(200);
        if (readLeftDistanceSensor() <= 10.0) {
            delay(200);
